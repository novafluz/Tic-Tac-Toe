 ...
 */
const std::string VERSION = "0.4.25022880";
...
struct GameSetup {
  ...
  int goal;
  GameMode mode;
  BotLevel levels[2]; // array to store 2 distincts lvs for EVE
};
...
void endGame(const RunConfig &config, GameSetup &gameSetup,
...
bool isValidMove(const char board[][BOARD_N_MAX], const int size, const int row,
                 const int col);
void makeMove(char board[][BOARD_N_MAX], const int row, const int col,
              const char symbol);
bool isEmptyHead(char board[][BOARD_N_MAX], int size, int x, int y,
                 const char symbol);
bool checkWin(char board[][BOARD_N_MAX], const int size, const char symbol,
              const int goal, EndRule rule = EndRule::OPEN_TWO);
 ...
 */
pII botMove(char board[][BOARD_N_MAX], const int size, const int goal,
            const char symbol, const BotLevel level);
...
void GameLogger::init(bool judge_mode, bool to_file, const std::string &path) {
  ...
  std::string header =
      "Tic-tac-toe Game (Version: " + std::string(VERSION) + ")\n";
...
RunConfig parseArgs(int argc, char *argv[]) {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (arg == "-j" || arg == "--judge") {
      ...
      config.input_file = argv[++i]; // after --input is a file, retrieve it

    } else if ((arg == "-l" || arg == "--log") && i + 1 < argc) {
      config.log_file = argv[++i];
      if (config.log_file == "") {
        config.to_file = false; // not write to file to avoid error.
      }
      ...
      std::cout << "Usage: \n"
                << "\t" << "--judge, -j" << "\t"
                << "Enable judge mode (no UI, standard output only)\n"
...
std::streambuf *initInteraction(const RunConfig &config) {
  ...
  if (!config.interactive && !config.input_file.empty()) {
    ...
    if (GameInteraction::global_file_in.is_open()) {
      ...
      GameLogger::log("failed to open input file, using console.",
...
bool validateInput(std::string &input) {
  ...
  for (char c : input) {
    if (!isdigit(static_cast<unsigned char>(c)))
      return false;
    // unsigned char takes only positive val 0->255, signed char takes -128->127
  }
  return true;
}
...
bool getInput(int *val) {
  ...
  std::string input;
  if (!(std::cin >> input) || !validateInput(input))
    return false;
  try {
    *val = std::stoi(input); // dereference val and assign integer value
    return true;
 ...
 */
bool selectGoal(int *goal, const int size) {
  // TODO: implement selecting game goal
  if (getInput(goal)) {
    if (*goal >= 3 && *goal <= size)
      return true;
  }
 ...
 */
bool selectGameMode(GameMode *mode) {
  // TODO: implement selecting game mode
  std::string input;
  if (std::cin >> input && validateInput(input)) {
    int select = stoi(input);
    if (select >= 1 && select <= 3) {
      *mode = static_cast<GameMode>(select - 1); // dereference and assign
      // mode now holds data type of GameMode.
      return true;
    }
  }
  return false;
 ...
 */
bool selectBotLevel(BotLevel *levels, const int index) {
  // TODO: implement selecting bot level
  std::string input;
  if (std::cin >> input && validateInput(input)) {
    int select = stoi(input);
    if (select >= 1 && select <= 3) {
      levels[index] = static_cast<BotLevel>(select - 1);
      // levels is a ptr to start, levels[index] move index forward
      // dereference that location and assign a new value
      return true;
    }
  }
  return false;
 ...
 */
bool getPlayerMove(int *row, int *col) {
  // Try to read two integers directly from input stream
  if (std::cin >> *row >> *col) {
    return true;
  }

  // If extraction fails, clear the error state and ignore bad input
  std::cin.clear();
...
void showPlayer(int player, bool is_bot) {
  // TODO:
  if (!is_bot)
    std::cout << std::format("Player {}'s turn", player) << std::endl;
  else
    std::cout << std::format("Bot (Player {}) is thinking...", player)
              << std::endl;
}
...
void showMove(const int row, const int col) {
  // TODO:
  std::cout << std::format("Move placed at ({}, {})", row, col) << std::endl;
}
...
void showInvalidMove() {
  // TODO:
  std::cout << "The move is invalid, please try again." << std::endl;
}
...
void showResult(const int winner, const bool is_bot) {
  ...
  if (winner == -1) {
    ...
    if (is_bot)
      ...
      std::cout << std::format("Player {} wins!", winner + 1) << std::endl;
  }
}
...
void startGame(const RunConfig &config, GameSetup &gameSetup) {
  ...
  while (true) {
    if (config.interactive) // for human
      showSelectMenu(SelectType::SIZE_UI);
    if (selectSize(&gameSetup.size)) { // pass &gameSetup.size into selectSize
                                       // for input
      GameLogger::log(std::format("Final size: {}", gameSetup.size));
      // log lv is already set default to INFO in namespace GameLogger
      break;
    } else {
      GameLogger::log("Invalid size input!", GameLogger::Level::ERROR);
    }
  }
  ...
  while (true) {
    ...
    if (selectGoal(&gameSetup.goal, gameSetup.size)) {
      GameLogger::log(std::format("Setup confirmed. Goal: {}, Size: {}",
                                  gameSetup.goal, gameSetup.size));
      break;
    } else {
      GameLogger::log("Invalid goal input! (Must be 3 <= goal <= size)",
                      GameLogger::Level::ERROR);
    }
  }
  // 5. Ask for game mode
  ...
  if (gameSetup.mode == GameMode::PVE) {
    while (true) {
      ...
      if (selectBotLevel(gameSetup.levels, 1)) {
        ...
        GameLogger::log(
            std::format("Bot level for player 2 selected : {}", levelStr));
        // log lv is already set default to INFO
        break;
      } else {
        GameLogger::log("Invalid bot level!", GameLogger::Level::ERROR);
      }
    }
  }
  ...
  if (gameSetup.mode == GameMode::EVE) {
    while (true) {
      if (config.interactive) {
        ...
        std::cout
            << "Enter levels for both bots: (e.g., '1 2' or '1' then '2'): \n";
      }
      if (selectBotLevel(gameSetup.levels, 0) &&
          selectBotLevel(gameSetup.levels, 1)) {
        ...
        GameLogger::log(std::format(
            "Bot level for player 1 ({}) and player 2 ({}) selected.",
            levelStr1, levelStr2));
        break;
      } else {
        GameLogger::log("Invalid bot level!", GameLogger::Level::ERROR);
  ...
  initBoard(gameSetup.board, gameSetup.size);
  // gameSetup.board will hold value after void initBoard later.
  GameLogger::log("Board setup completed.");
}
...
GameResult playGame(const RunConfig &config, GameSetup &gameSetup) {
  ...
  while (true) {
    ...
    if (gameSetup.mode == GameMode::EVE)
      ...
      is_bot = (currentPlayer == 1);

    if (config.interactive)
      showPlayer(currentPlayer + 1, is_bot);
    ...
    if (!is_bot) {
      while (true) {
        ...
        if (getPlayerMove(&move.first, &move.second) &&
            isValidMove(gameSetup.board, gameSetup.size, move.first,
                        move.second))
          break;
        if (config.interactive)
          showInvalidMove();
    ...
    //      d) validate move
    //         isValidMove(...)
    if (!isValidMove(gameSetup.board, gameSetup.size, move.first,
                     move.second)) {
      GameLogger::log("Invalid move!", GameLogger::Level::WARNING);
      continue;
    }
    ...
    //      f) check win
    //         checkWin(...)
    if (checkWin(gameSetup.board, gameSetup.size, symbol[currentPlayer],
                 gameSetup.goal)) {
...
void endGame(const RunConfig &config, GameSetup &gameSetup,
  ...
  std::string winnerStr = (gameResult.winner == DRAW_RESULT) ? "Draw"
                          : (gameResult.winner == 0)         ? "Player 1"
                                                             : "Player 2";
  GameLogger::log(
      std::format("Game over. Winner: {}. isBot: {}. Total moves: {}",
                  winnerStr, gameResult.isBot, gameResult.turns));
}
...
void initBoard(char board[][BOARD_N_MAX], const int size) {
  ...
  // Initialize the playable board area to '-' and leave outside cells
  // unchanged.
  for (int i = 0; i < size; i++) {
  ...
  GameLogger::log("Board initiated!", GameLogger::Level::DEBUG);
}
...
bool checkWin(char board[][BOARD_N_MAX], const int size, const char symbol,
  ...
  // Direction vectors for: Right (0,1), Down (1,0)
  // -> main diag (1,1), anti diag (1,-1)
  int dRow[] = {0, 1, 1, 1};
  int dCol[] = {1, 0, 1, -1};
  for (int i{0}; i < size; i++) {
    for (int j{0}; j < size; j++) {
      if (board[i][j] != symbol)
        continue;
      // only check if current cell belongs to player
      for (int d{0}; d < 4; d++) { // check directions
        bool chainStt = true;
        for (int k{1}; k < goal; k++) { // check chain 1 -> goal
          int ni = i + k * dRow[d];     // next rows
          int nj = j + k * dCol[d];
        ...
        int head_x = i - dRow[d];
        int head_y = j - dCol[d];
        int tail_x = i + goal * dRow[d];
        int tail_y = j + goal * dCol[d];
        bool h = isEmptyHead(board, size, head_x, head_y, symbol);
        bool t = isEmptyHead(board, size, tail_x, tail_y, symbol);
        switch (rule) {
        case EndRule::NONE:
          return true;
...
pII random_pick(char board[][BOARD_N_MAX], const int size) {
  ...
  while (!isValidMove(board, size, move.first, move.second)) {
    move.first = dist(generator);
    move.second = dist(generator);
  }
  return move;
}
...
pII simple_heuristic(char board[][BOARD_N_MAX], const int size, const int goal,
                     const char botSymbol, const char playerSymbol) {
  // TODO: student implementation
  // loop for bot first, if possibly win -> hit
  for (int i = 0; i < size; i++) {
    for (int j = 0; j < size; j++) {
      if (board[i][j] == '-') {
        board[i][j] = botSymbol;
...
int main(int argc, char *argv[]) {
  // argc is arg counts, argv is an array of char* []
